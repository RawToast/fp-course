...
flattenAgain ::
  List (List a)
  -> List a
flattenAgain as = flatMap(\a -> a) as
...
seqOptional ((Full x) :. xs) = mapOptional(\bs -> (x :. bs)) (seqOptional xs)
-- This is interesting
...
lengthGT4 ::
  List a
  -> Bool
lengthGT4 xs = countToFour xs 0

countToFour ::
  List a
  -> Int
  -> Bool
countToFour Nil i = if i > 4 then True else False
countToFour (_ :. xs) i = if i > 4 then True else countToFour xs (i + 1)
...
reverse ::
  List a
  -> List a -- \bs -> (x :. bs)
reverse as = foldLeft(\t h -> (h :. Nil) ++ t) Nil as
